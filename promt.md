...
```
- Таблица пользователей уже создана и она называется moonshine_users, если просят привязывать пользователей к ресурсу то сделай следующим образом:
```json
{
...
```
### Самый последний шаг
Ты должен обязательно его выполнить. После того как ты сгенерировал схему, выполни проверку её структуры на соответствие MAIN_SCHEMA, особенно обрати внимание на пункты oneOf и на регистр букв, всё должно следовать именно MAIN_SCHEMA. Исправь все допущенные ошибки. Если твой ответ начинается с ```json и заканчивается ```, удали эти символы.